...
const Dashboard = () => {
  ...
  const getUserStatusBadgeColor = (isActive) => {
  ...
  // Check if staff member is current logged-in user
    ...
    <>
      ...
      <div className="container-fluid py-3 py-md-4 bg-light min-vh-100">
        ...
        {activeTab === "overview" ? (
                  ...
                  <div className="card-body p-3 p-md-4">
                    ...
                    <div className="text-center p-3">
                      ...
                      <div className="d-flex justify-content-center gap-3 mt-3">
                        <div>
                          <span className="badge bg-danger">Admin: {staffMembers.filter(m => m.role === 'admin' && m.isActive).length}</span>
                        </div>
                        <div>
                          <span className="badge bg-primary">Staff: {staffMembers.filter(m => m.role === 'staff' && m.isActive).length}</span>
                        </div>
                        <div>
                          <span className="badge bg-secondary">Inactive: {staffMembers.filter(m => !m.isActive).length}</span>
                        </div>
                      </div>
                    </div>
                  </div>
          ...
          <div className="row">
            <div className="col-12">
              <div className="card border-0 shadow-sm custom-scrollbar">
            ...
            <div className="col-12 col-md-4">
              <div className="card border-0 shadow-sm h-100">
                <div className="card-body p-3 p-md-4">
                  ...
                  <div className="table-responsive table-scrollbar" style={{ maxHeight: "200px" }}>
                    <table className="table table-sm table-hover mb-0">
                      ...
                      <tbody>
                        {vendors.slice(0, 5).map((vendor) => (
                          <tr key={vendor._id}>
                            <td className="small text-truncate" style={{ maxWidth: "80px" }}>
                              <div className="d-flex flex-column">
                                <span className="fw-medium">{vendor.name}</span>
                                <small className="text-muted d-sm-none">{vendor.companyName}</small>
                              </div>
                            </td>
                            <td className="small text-truncate d-none d-sm-table-cell" style={{ maxWidth: "100px" }}>
                              {vendor.companyName}
                            </td>
                            <td>
                              <span className={`badge bg-${getStatusBadgeColor(vendor.status)}`}>
                                {vendor.status || 'N/A'}
                              </span>
                            </td>
                      ...
                      <tbody>
                        {categories.slice(0, 5).map((category) => (
                          <tr key={category._id}>
                            ...
                            <td className="small">
                              <span className={`badge bg-${getStatusBadgeColor(category.status)}`}>
                                {category.status || 'N/A'}
                              </span>
                            </td>
                            <td className="text-end" style={{ width: "100px" }}>
                    ...
                    <table className="table table-sm table-hover mb-0">
                      ...
                      <tbody>
                        {staffMembers.map((member) => {
                          ...
                          return (
                            <tr key={member._id} className={isCurrent ? "table-active" : ""}>
                              <td className="small text-truncate" style={{ maxWidth: "80px" }}>
                                <div className="d-flex align-items-center">
                                  <span className="fw-medium">{member.name}</span>
                                  {isCurrent && (
                                    <span className="badge bg-info ms-2" title="Current User">
                              ...
                              <td className="small">
                                <span className={`badge bg-${getUserStatusBadgeColor(member.isActive)}`}>
                              ...
                              <td className="text-end" style={{ width: "100px" }}>
                                <div className="btn-group btn-group-sm">
                                  <button
                                    ...
                                    title={isCurrent ? "Edit My Profile" : "Edit Staff Member"}
        ...
        {showStaffForm && (
          <StaffMemberForm
            ...
            isCurrentUser={isCurrentUser(editingStaff?._id)}
